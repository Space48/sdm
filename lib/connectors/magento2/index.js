import Magento2, { configSchema } from "./client";
...
export const magento2 = connector({
    ...
    resources: {
        categories: mergeResources(endpoint.crud("categories", {
            idField: "id",
            list: {
                uri: "categories/list",
        ...
        products: mergeResources(endpoint.crud("products", {
            idField: "sku",
            list: {
                sortKey: { query: "entity_id", response: "id" },
            },
            ...
            resources: {
                attributes: mergeResources(endpoint.crud("products/attributes", {
                ...
                attributeSets: mergeResources(endpoint.crud("products/attribute-sets", {
...
function getScopeName(storeBaseUrl, storeView = "default") {
    const { host, path } = parseUrl(storeBaseUrl);
    return `${host || ""}${path}`.replace(/\/$/, "").toLowerCase() + `/${storeView}`;
}
