...
(function (Rest) {
    function crud(uriPattern, childNames = []) {
        ...
        return {
            endpoints: {
                create: Rest.create(uriPattern),
                list: Rest.list(uriPattern),
            },
            documents: {
                idField: 'entity_id',
                listIds: listIds(uriPattern),
                endpoints: {
                    delete: Rest.del(docUriPattern),
                    get: Rest.get(docUriPattern),
                    update: Rest.update(docUriPattern),
                },
    ...
    function children(parentUri, children) {
        return ramda_1.default.fromPairs(children.map(childName => ramda_1.default.pair(childName, framework_1.resource({
            endpoints: {
                get: Rest.get(`${parentUri}/childName`)
            },
        }))));
    }
    ...
    Rest.list = (uriPattern) => fn(uriPattern, (m2Client, uri) => m2Client.search(uri, { sortKey: 'entity_id' }));
