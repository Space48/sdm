...
function endpointDefinitions(resourceKey, keys, config, target) {
    try {
        return ramda_1.default.filter(Boolean, ramda_1.default.fromPairs(keys.map(endpointKey => [
            endpointKey,
...
function endpointDefinition(resourceKey, endpointKey, config, target) {
    ...
    if (resolvedConfig.type === "flatMap") {
        return scope => ({ path, input }) => scope.flatMap(resourceKey, endpointKey, framework_1.Path.getDocIds(path), {
            ...resolvedConfig.params,
            ...input,
        });
    }
    return scope => ({ path, input }) => scope.map(resourceKey, endpointKey, framework_1.Path.getDocIds(path), {
        ...resolvedConfig.params,
        ...input,
    });
}
