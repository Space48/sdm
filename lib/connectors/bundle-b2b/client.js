...
class BundleB2b {
    ...
    async *list(uri, params) {
        ...
        for (let page = 2; page <= totalPages; page += concurrency) {
            const pages = await Promise.all(threads.map(threadId => this.get(uri, { offset: ((page + threadId) * limit) - limit, ...(params || {}) })));
    ...
    async getToken(config) {
        ...
        const tokenResponse = await node_fetch_1.default("https://api.bundleb2b.net/api/io/auth/backend", {
            headers: {
                "Content-Type": "application/json",
            },
            body: JSON.stringify({
