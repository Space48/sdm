...
async function runNonInteractiveMode() {
    ...
    if (process.stdin.isTTY || command.input !== undefined) {
        ...
        await json_pipe_1.pipe(json_pipe_1.readJsonLinesFrom(process.stdin), framework_1.Stateful.map((input) => ({ ...command, input })), commands => scope.execute(commands), json_pipe_1.writeJsonLinesTo(process.stdout));
    }
}
