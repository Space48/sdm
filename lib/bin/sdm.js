#!/usr/bin/env node
import { EndpointError, Path, State, } from "../framework";
...
async function runHelpMode() {
    ...
    process.stderr.write(Shell.explainCliUsageOnCli(connector.$definition, {
...
async function runNonInteractiveMultiMessageMode(scopeRef) {
    const scope = await app.requireScope(scopeRef);
    await pipe(readJsonLinesFrom(process.stdin), commands => scope.execute(commands), writeJsonLinesTo(process.stdout));
}
async function runNonInteractiveSingleMessageMode(header) {
    const command = {
        ...header,
        input: argsExcludingFlags.length > 1
    ...
    if (process.stdin.isTTY || command.input !== undefined) {
        try {
            ...
            console.error(e instanceof EndpointError ? JSON.stringify({ error: e.normalize() }, null, 2) : e);
            process.exit(1);
        }
    }
    else {
        await pipe(readJsonLinesFrom(process.stdin), State.map((input) => ({ ...command, input })), commands => scope.execute(commands), writeJsonLinesTo(process.stdout));
    }
}
    ...
    for (;;) {
        ...
        try {
            ...
            if (!interrupted) {
                ...
                await pipe(scope.execute(command), tap(() => {
                    numOutputs++;
                    return new Promise(setImmediate);
                }), takeWhile(() => !interrupted), map(item => `${JSON.stringify(item)}\n\n`), // blank line to improve readability in interactive mode
...
async function warnUserIfNecessary(header) {
    ...
    process.stderr.write(chalk.yellow(interactiveMode
