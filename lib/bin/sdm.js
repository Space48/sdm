...
async function runNonInteractiveMode() {
    ...
    if (process.stdin.isTTY || command.input !== undefined) {
        await json_pipe_1.streamJson(scope.execute(command));
    }
    else {
        await json_pipe_1.transformJson(json_pipe_1.compose(json_pipe_1.map((input) => ({ ...command, input })), commands => scope.execute(commands)));
    }
}
    ...
    while (true) {
        ...
        try {
            ...
            if (!interrupted) {
                ...
                }), json_pipe_1.takeWhile(() => !interrupted), json_pipe_1.streamJson);
