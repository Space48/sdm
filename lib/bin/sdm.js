...
async function main() {
    try {
        ...
        if (argsExcludingFlags[0]) {
            const [scope, path] = framework_2.BinaryApi.decodeScopeAndPath(argsExcludingFlags[0]);
...
async function runNonInteractiveMode() {
    const [scopeRef, path] = framework_2.BinaryApi.decodeScopeAndPath(argsExcludingFlags[0]);
    if (!scopeRef) {
        throw new Error();
    }
    const command = {
    ...
    if (process.stdin.isTTY || command.input !== undefined) {
        ...
        await json_pipe_1.pipe(json_pipe_1.readJsonLinesFrom(process.stdin), json_pipe_1.map((input) => ({ ...command, input })), commands => scope.execute(commands), json_pipe_1.writeJsonLinesTo(process.stdout));
    }
}
    ...
    while (true) {
        ...
        try {
            ...
            if (!interrupted) {
                ...
                }), json_pipe_1.takeWhile(() => !interrupted), json_pipe_1.writeJsonLinesTo(process.stdout));
...
async function askForCommand(scopeRef) {
    ...
    while (true) {
        ...
        const commandLine = await ask({
            question: `sdm ${framework_2.BinaryApi.encodeScope(scopeRef)}> `,
            completer: (line) => {
...
function suggestCommands(connector, line) {
    try {
        ...
        const path = framework_2.BinaryApi.decodePartialPath(line);
        const lastElement = path.slice(-1)[0];
        if (Array.isArray(lastElement)) {
            const host = selectHost(path);
            return framework_1.Path.computeAll(host, path)
                .map(framework_2.BinaryApi.encodePath);
        }
        else {
            const hostPath = path.slice(0, -1);
            const host = hostPath.length === 0 ? connector : selectHost(hostPath);
            return framework_1.Path.computeAll(host, hostPath)
                .map(framework_2.BinaryApi.encodePath)
...
async function getAvailableScopes() {
    return (await app.listScopes()).map(framework_2.BinaryApi.encodeScope);
}
const safeEndpoints = ['get', 'list'];
async function warnUserIfNecessary(scopeRef, command) {
    const [, endpointName] = framework_1.Path.popEndpointName(command.path);
    ...
    const commandHeader = framework_2.BinaryApi.encodeCommandHeader(command);
