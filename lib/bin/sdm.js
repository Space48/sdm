...
async function runNonInteractiveMultiMessageMode(scopeRef) {
    ...
    await json_pipe_1.pipe(json_pipe_1.readJsonLinesFrom(process.stdin), commands => scope.execute(commands), json_pipe_1.writeJsonLinesTo(process.stdout));
}
async function runNonInteractiveSingleMessageMode(header) {
    const command = {
        ...header,
        input: argsExcludingFlags.length > 1
            ? framework_2.BinaryApi.decodeCommandInput(argsExcludingFlags[1])
            : undefined,
    };
    ...
    if (process.stdin.isTTY || command.input !== undefined) {
        try {
            ...
            console.error(e instanceof framework_1.EndpointError ? JSON.stringify({ error: e.normalize() }, null, 2) : e);
            process.exit(1);
        }
    }
    else {
...
function suggestCommands(connector, line) {
    try {
        ...
        if (Array.isArray(lastElement)) {
            ...
            return framework_1.Path.computeAllHeaders(host, path).map(framework_2.BinaryApi.encodeHeader);
        }
        else {
