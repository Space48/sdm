...
function main() {
    ...
    Object.entries(connectors_1.regularConnectors).forEach(([name, connector]) => {
        ...
        summaryMarkdown = summaryMarkdown.concat(docgen_1.Markdown.explainSummary(connector.$definition, name));
    });
    fs_1.writeFileSync(path_1.default.join(docsDir, "summary.md"), summaryMarkdown);
}
