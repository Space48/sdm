...
exports.Stateful = exports.EndpointError = exports.InvalidCommand = exports.InvalidConnectorDefinition = exports.MutableReference = exports.Reference = exports.Path = exports.resourceMerger = exports.resource = exports.connector = void 0;
            ...
            return {
                ...
                async *execute(commandOrCommands) {
                    if (isIterable(commandOrCommands)) {
                        yield* json_pipe_1.pipe(executeCommands(commandOrCommands), Stateful.map(result => ({
                            ...result,
                        ...
                        if (pathContainsWildcard) {
                            yield* json_pipe_1.pipe(outputElements, Stateful.tap(({ success, error }) => {
                                if (!success) {
...
function commandExecutor(resources, scope) {
    ...
    return json_pipe_1.compose(ensureIterableIsAsync, Stateful.flatMapAsync({ concurrency: 10 }, command => json_pipe_1.pipe(expandPaths(command.path), json_pipe_1.map(path => ({ ...command, path })))), Stateful.flatMapAsync({ concurrency: 100 }, async function* (command) {
...
class Stateful {
    constructor() { }
    static map(mapper) {
        return json_pipe_1.map(element => 'state' in element ? element : mapper(element));
    }
    static tap(fn) {
        return json_pipe_1.tap(element => 'state' in element ? element : fn(element));
    }
