...
export const magento1 = connector({
  ...
  resources: {
      ...
      documents: {
        ...
        endpoints: {
          getSoap:
            ({ soap }) =>
            ({ docId: [categoryId] }) =>
              soap.execute("catalogCategoryInfo", {
                categoryId,
                storeView: "default",
              }),
        },
        ...
        tree: {
          endpoints: {
            getSoap:
              ({ soap }) =>
              () =>
                soap.execute("catalogCategoryTree"),
          },
        },
      },
      ...
      documents: {
          ...
          addresses: {
            endpoints: {
              getSoap:
                ({ soap }) =>
                ({ docId: [customerId] }) =>
                  soap.execute("customerAddressList", { customerId }),
            },
          },
        },
        ...
        endpoints: {
          getSoap:
            ({ soap }) =>
            ({ docId: [customerId] }) =>
              soap.execute("customerCustomerInfo", { customerId }),
        },
      },
    ...
    orders: mergeResources(Rest.read("orders", ["addresses", "comments", "items"] as const), {
      endpoints: {
        listSoap: Soap.list("salesOrderList"),
      },
    }),

    products: mergeResources(Rest.crud("products", ["categories", "images", "websites"] as const), {
      documents: {
        resources: {
          links: {
            documents: {
              ...
              endpoints: {
                getSoap:
                  ({ soap }) =>
                  ({ docId: [product, type] }) =>
                    soap.execute("catalogProductLinkList", { product, type }),
              },
            },
          },
          media: {
            endpoints: {
              getSoap:
                ({ soap }) =>
                ({ docId: [productId] }) =>
                  soap.execute("catalogProductAttributeMediaList", { productId }),
            },
          },
        },
        ...
        endpoints: {
          getSoap:
            ({ soap }) =>
            ({ docId: [productId] }) =>
              soap.execute("catalogProductInfo", { productId }),
        },
      },
            ...
            listSoap: ({ soap }) =>
              compose(
                ...
                flatMapAsync(async function* (setId) {
                  const result = await soap.execute<any[]>("catalogProductAttributeList", {
                    setId,
                  });
                  yield* result ?? [];
                }),
                ...
                flatMap(attributes =>
                  attributes.sort((attr1, attr2) => attr1.attribute_id - attr2.attribute_id),
                ),
          ...
          documents: {
            ...
            endpoints: {
              getSoap:
                ({ soap }) =>
                ({ docId: [attribute] }) =>
                  soap.execute("catalogProductAttributeInfo", { attribute }),
            },
          },
        },
